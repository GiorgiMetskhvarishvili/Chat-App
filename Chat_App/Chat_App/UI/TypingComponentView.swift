...
class TypingComponentView: UIView {
  ...
  private var sendButtonImageName: String
  ...
  private func updateTextViewHeight() {
    ...
    let maxHeight = textView.font?.lineHeight ?? 0 * CGFloat(maxNumberOfLines)
    let size = textView.sizeThatFits(CGSize(width: textView.frame.width, height: maxHeight))
    if size.height <= maxHeight {
      textView.isScrollEnabled = false
      containerView.frame.size.height = size.height + ContainerViewConstants.containerTopAndBottomPadding
    } else {
      textView.isScrollEnabled = true
      containerView.frame.size.height = maxHeight + ContainerViewConstants.containerTopAndBottomPadding
    }
